...
@section('content')
		...
		<table class="table p-0 m-0">
			<tr class="p-0">
				<td width="44%" class="pl-1 pr-1 pt-1 pb-0 border-0">
					<h6>Cursos Pendientes por Evaluar
	...
	<div class="container-fluid col-lg-12 col-md-12 col-12 text-light bg-dark border border-info">
		<table class="table p-0 m-0 w-100">
			<tr>
				<td width = "1%"  class="border-0 p-0 m-0 text-center">
					ID
				</td>
				<td width = "51%" class="border-0 p-0 m-0 text-center">
					Descripción del Adiestramiento
				</td>
				<td width = "9%"  class="border-0 p-0 m-0 text-center">
					Enviado
				</td>
		...
		<table id="list" class="table table-striped table-hover w-100" style="overflow-x: hidden; overflow-y: auto">
			@foreach ($cursos as $curso)
				<tr id="r{{ $curso->id }}" class="@if($curso->suspendido==1) edo_suspendido @endif">
					<td width = "3%"  style="text-align: center;" id="id{{ $curso->id }}" 
					...
					<td width = "7%" style="text-align: center;"
						...
						<input class="txtLabel @if($curso->suspendido==1) txtLabelSus @endif"
							...
							type="text" value="{{ date('d-m-Y', strtotime($curso->enviado)) }}">
					</td>
					...
					<td width = "10%" style="text-align: center;"
						class="border border-primary border-top-0 border-left-0 border-right-0">
						@if($curso->intento == NULL) --- @else {{ $curso->intento }} @endif
					</td>
					<td width = "10%" style="text-align: right; vertical-align: middle; font-size: 18px;"
						...
						@if($_SESSION['tipo_usr'] == 2)
							<a href="/participantesporevaluar/{{ $curso->id }}"	id="evaluar">
							<i class="fas fa-user-tag fa-lg img_btn"
								title="Participantes a Evaluar"></i></a>
						@else
							<a href="#" data-toggle="modal" data-target="#ModalVista"
								id="verparticipantes" data-cid="{{ $curso->id }}">
								<i class="fas fa-search fa-lg img_btn"
									title="Ver Participantes"></i></a>
						@endif
					</td>
				</tr>
			@endforeach
